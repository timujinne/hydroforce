...
export const Hero: React.FC<HeroProps> = ({ 
  ...
  if (overlayType === 'dark') {
    overlayGradient = 'linear-gradient(rgba(2, 24, 34, 0.85), rgba(0, 58, 82, 0.85))';
  } else if (overlayType === 'light') {
    overlayGradient = 'linear-gradient(rgba(248, 249, 250, 0.85), rgba(233, 236, 239, 0.85))';
    ...
    >
      <div className="container mx-auto px-4 relative z-10">
        <div className={`max-w-4xl ${textColor}`}>
          <h1 className={`text-4xl md:text-5xl lg:text-7xl font-bold mb-6 ${textShadowClass} leading-tight tracking-tight`}>
            {title}
          </h1>
          <p className={`text-xl md:text-2xl font-light mb-8 opacity-95 ${textShadowClass} leading-relaxed max-w-2xl`}>
            {subtitle}
          </p>
          {description && (
            <p className={`text-lg md:text-xl mb-10 opacity-90 max-w-3xl ${textShadowClass} leading-relaxed font-normal`}>
              {description}
            </p>
          )}
          ...
          {stats && (
            <div className="flex flex-wrap gap-6 mb-12">
              {stats.map((stat, index) => (
                ...
                >
                  <strong className={`block text-3xl md:text-4xl font-bold mb-1 ${overlayType === 'light' ? 'text-accent' : 'text-white drop-shadow-md'}`}>
                    {stat.value}
                  </strong>
                  <span className={`text-sm md:text-base font-bold uppercase tracking-wider ${overlayType === 'light' ? 'text-text-gray' : 'text-gray-100'}`}>
                    {stat.label}
                  </span>
                </div>
              ))}
