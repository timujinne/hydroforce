...
export const CylinderForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Use the same Formspree endpoint as the Contact Form in Layout.tsx
    const FORMSPREE_ENDPOINT = "https://formspree.io/f/xqanlwzb"; 

    try {
      const response = await fetch(FORMSPREE_ENDPOINT, {
        method: 'POST',
        body: formData, // Formspree handles FormData automatically (including checkboxes/arrays)
        headers: {
      ...
      if (response.ok) {
        setStatus('success');
        form.reset();
        setVisibility({}); // Reset conditional fields visibility
          ...
          <div className="form-content">
            ...
            {status === 'error' && (
                <div className="error-message">
                    ✕ An error occurred during submission. Please try again or contact us directly at office@hydroforce.ee.
                </div>
            )}

            <form onSubmit={handleSubmit}>
              {/* Hidden Fields for Formspree Configuration */}
              <input type="hidden" name="_subject" value="New Hydraulic Cylinder Specification" />
                ...
                <div className="hf-form-group">
                  ...
                  <div className="hf-checkbox-group">
                    {[
                        ...
                        {id: 'eye', label: 'Eye with Bushing'},
                        {id: 'fork', label: 'Fork'},
                        {id: 'sphere', label: 'Sphere / Ball Joint'}
                    ].map(item => (
                        <div className="hf-checkbox-item" key={item.id}>
                            <input type="checkbox" name="rod_mount" value={item.id} id={`rod_${item.id}`} />
                            <label htmlFor={`rod_${item.id}`}>{item.label}</label>
                        </div>
                    ))}
                    <div className="hf-checkbox-item">
                      <input 
