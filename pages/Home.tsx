  ...
  return (
    <>
      <Hero
        ...
        buttons={[
          { label: "Explore Technologies", link: "#technology-solutions", type: "primary" },
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 max-w-3xl mx-auto">
            ...
            <p className="text-xl text-gray-500">30 years of supplying high-precision components to key industries worldwide</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
             {industries.map((ind, idx) => (
               <div key={idx} className="bg-white p-8 rounded-xl shadow-card hover:shadow-card-hover transition-all duration-300 border-l-4 border-primary hover:border-accent hover:-translate-y-1 h-full flex flex-col justify-start">
                 <h3 className="text-xl font-bold text-text-gray mb-3">{ind.title}</h3>
                 <p className="text-gray-600 leading-relaxed text-base">{ind.desc}</p>
               </div>
             ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-gradient-to-br from-accent to-accent-dark text-center text-white relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Ready to Partner with HydroForce?</h2>
          <p className="text-xl opacity-90 mb-8 max-w-2xl mx-auto">Contact our engineering team for technical consultation and custom manufacturing solutions</p>
          <HashLink smooth to="/#contacts" className="inline-block px-8 py-4 bg-white text-accent font-bold rounded-lg shadow-lg hover:bg-bg-light hover:-translate-y-1 transition-all">
            Request Quote
          </HashLink>
        </div>
      </section>
      ...
      <section id="technology-solutions" className="py-20 bg-bg-light">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">Hydroforce Technology Solutions</h2>
            <p className="text-xl text-gray-500">Four core technologies for comprehensive manufacturing solutions</p>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">Our Manufacturing Excellence</h2>
            <p className="text-xl text-gray-500">Precision engineering and quality in every detail</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-auto lg:h-[500px]">
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-xl group h-[400px] lg:h-full bg-white border border-gray-100 p-4">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-primary/95 to-primary/80 p-8 text-white translate-y-0 transition-all">
                ...
                <p className="text-base opacity-90">Custom hydraulic solutions for any applications</p>
              </div>
            </div>
      ...
      {/* Quality Section */}
      <section className="py-24 bg-cover bg-center bg-fixed relative text-white" style={{ backgroundImage: `linear-gradient(rgba(1, 87, 125, 0.95), rgba(1, 58, 82, 0.95)), url('https://www.hydroforce.ee/wp-content/uploads/2025/10/Back-for-HC.webp')` }}>
        <div className="container mx-auto px-4 relative z-10">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
             <div>
               <h2 className="text-3xl font-bold mb-6 border-b-2 border-accent inline-block pb-2">Quality at Every Step</h2>
               <p className="mb-6 text-gray-200 leading-relaxed text-lg">Quality isn't just our promise—it's our foundation. Our modern facilities feature cutting-edge testing equipment ensuring every product represents the pinnacle of market excellence.</p>
               <ul className="space-y-3">
                 {['ISO 9001:2015 certified', 'Advanced CMM equipment', 'Full traceability', 'Continuous improvement'].map((item, i) => (
                   <li key={i} className="flex items-center gap-3 text-lg"><span className="text-accent font-bold text-xl">•</span> {item}</li>
                 ))}
               </ul>
             </div>
             <div>
               <h2 className="text-3xl font-bold mb-6 border-b-2 border-accent inline-block pb-2">Responsible Manufacturing</h2>
               <p className="mb-6 text-gray-200 leading-relaxed text-lg">Environmental responsibility drives our operations. We've implemented eco-friendly practices that reduce waste and minimize environmental impact.</p>
               <ul className="space-y-3">
                 {['Waste reduction (-25%)', 'Energy-efficient processes', 'Recyclable materials', 'Carbon footprint reduction'].map((item, i) => (
                   <li key={i} className="flex items-center gap-3 text-lg"><span className="text-accent font-bold text-xl">•</span> {item}</li>
                 ))}
               </ul>
             </div>
      ...
      <section className="py-20 bg-bg-light">
        <div className="container mx-auto px-4">
           <div className="text-center mb-12">
              ...
              <p className="text-xl text-gray-500">Trends and innovations in metallurgy and manufacturing</p>
           </div>
...
const industries = [
  { title: "Automotive Excellence", desc: "Precision components for automotive industry: from engine parts to hydraulic systems. Supporting OEMs and Tier 1 suppliers." },
  { title: "Heavy Machinery", desc: "Robust hydraulic cylinders and precision components for heavy construction equipment. Custom solutions for excavators and loaders." },
  { title: "Agricultural Solutions", desc: "Durable solutions for agricultural machinery: hydraulics and metal components designed for harsh field conditions." },
  { title: "Industrial Precision", desc: "High-precision parts for industrial equipment and automation systems. Delivering accuracy for modern manufacturing." },
  { title: "Energy Innovation", desc: "Components for energy equipment with enhanced reliability. Supporting renewable energy, oil & gas, and power generation." },
  { title: "Marine Durability", desc: "Corrosion-resistant solutions for marine and offshore applications. Specialized alloys for harsh maritime environments." },
];

const TechCard: React.FC<{title: string, subtitle: string, description: string, specs: string[], link: string}> = ({ title, subtitle, description, specs, link }) => (
  <div className="bg-white rounded-xl p-8 shadow-card hover:shadow-card-hover transition-all duration-300 flex flex-col h-full">
    ...
    <div className="mt-auto">
      <h5 className="font-bold text-sm text-text-gray mb-3 uppercase tracking-wider">Specifications</h5>
      ...
      <Link to={link} className="inline-flex items-center text-accent font-bold hover:text-primary transition-colors">
        Learn more &rarr;
      </Link>
    </div>
  </div>
...
const GallerySmallItem: React.FC<{img: string, title: string, desc: string}> = ({ img, title, desc }) => (
  <div className="relative rounded-xl overflow-hidden shadow-md group bg-white h-full border border-gray-100 p-4">
    ...
    <div className="absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/40 to-transparent flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      <h3 className="text-white font-bold text-lg translate-y-4 group-hover:translate-y-0 transition-transform duration-300">{title}</h3>
      <p className="text-white/90 text-base translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-75">{desc}</p>
    </div>
  </div>
);

const NewsCard: React.FC<{img: string, title: string, desc: string, link: string}> = ({ img, title, desc, link }) => (
  <Link to={link} className="group bg-white rounded-xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 flex flex-col h-full">
     ...
     <div className="p-6 flex-1 flex flex-col">
        ...
        <span className="text-accent font-bold text-sm uppercase tracking-wide group-hover:translate-x-2 transition-transform inline-block">Read Article &rarr;</span>
     </div>
  </Link>
);
